name    'puppetlabs-openstack'
version '1.1.0'
source  'https://github.com/puppetlabs/puppetlabs-openstack'
author  'Puppet Labs'
license 'Apache License 2.0'
summary 'Puppet Labs Openstack Module for folsom'
description 'Module to install common openstack-essex configurations using puppet'
project_page 'https://github.com/puppetlabs/puppetlabs-openstack'

dependency 'puppetlabs/glance', '>= 1.1.0'
dependency 'puppetlabs/horizon', '>= 1.1.0'
dependency 'puppetlabs/keystone', '>= 1.1.1'
dependency 'saz/memcached', '>= 2.0.2'
dependency 'puppetlabs/mysql','>= 0.5.0'
dependency 'puppetlabs/nova', '>= 1.1.1'
dependency 'puppetlabs/cinder', '>= 1.1.1'
#dependency 'ekarlso/quantum', '>= 0.2.2'
